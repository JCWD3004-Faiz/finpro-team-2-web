...
import {
  ...
  setStoreId,
  setProductId,
} from "@/redux/slices/storeSalesSlice";
...
function StoreAllReports() {
  ...
  const { storeName, storeLocation, adminName, isSidebarOpen } = useSelector(
  ...
  const {
    ...
    category_id,
    product_id,
  } = useSelector((state: RootState) => state.storeSales);
  ...
  return (
    <div className="bg-slate-100 w-screen min-h-screen text-gray-800">
      ...
      <div className={`ml-0 ${isSidebarOpen ? "md:ml-64" : ""} md:ml-64 p-6`}>
        <h1 className="text-4xl font-semibold text-gray-900 mb-5 tracking-wide">
          Sales Report
        </h1>
        <div className="mb-5">
        ...
        <Button
          size="default"
          onClick={() => router.push("/admin-store/reports/stocksReport")}
        >
          <TbReportSearch />
          Stocks Report
        </Button>
        ...
        <div className="flex gap-5 mt-6 mb-4">
          <div className="bg-white w-1/3 rounded-sm">
            <Select
...
export default StoreAllReports;
